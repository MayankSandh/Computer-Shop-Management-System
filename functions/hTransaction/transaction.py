...
class Transaction:
    def __init__(self):
        ...
        self.products = ["processor", "monitors", "graphics card",'memory chips','storage'] 
        # !! The names in the self.products list much match with the CSV FILENAMES in the hardware folder
            ...
            if self.items_ordered == []:
                ...
                if input('Do you wish to continue your purchases(y/n): ').lower()=='n':
                    ...
                    with open("transactions/{}.txt".format(t), "w") as file:
                        file.write('Transaction type: Hardware'+'\n')
    ...
    def menu1(self): # this is the first sub-menu (menu1) under PRODUCT CATEGORIES
        ...
        print(50*"~")
        # we are accessing the csv file inside hardware folder with the name/category chosen in menu0
        with open("hardware/{}.csv".format(self.products[self.choice-1])) as file:
    ...
    def menu2(self):
        ...
        if self.choice3 == 0: # goes back to menu1
            ...
            if input("Do you want to go back? (y/n):").lower() == "y":
                ...
                self.menu2()
    # def cash_memo(self): # this is an intermediate unfinished func to show how the items ordered can be accessed and manipulated
    # # this function can later be replaced by a central cash_memo function
    #     print("\033c")
    #     print("~"*50)
    #     print("{:40s}{:10s}".format("ITEMS_ORDERED", "PRICE"))
    #     print("~"*50)
    #     amt = 0
    #     for item in self.items_ordered:
    #         print("{:<40s}{:<10d}".format(item[0][:30]+"...", item[1]))
    #         amt+=int(item[1])
    #     print("~"*50)
    #     print("{:>40s}{:<10d}".format("TOTAL AMOUNT:  ", amt))
    #     print()
